// Pure iso renderer — no DOM refs, accepts mission data directly.
// Shared between editor-map and editor-preview pages.
    ...
    if (mode === 'filled') {
        ...
        foliage.forEach((f: any) => {
            ...
            if (f.type === 'pine') {
                ...
                tCtx.beginPath();
                tCtx.ellipse(cp.x - sTW * 0.2 * s, cp.y - sTH * 0.3 * s, cr * sTW * 0.7, cr * sTH * 0.7, 0, 0, Math.PI * 2);
                tCtx.fill();
            } else if (f.type === 'bush') {
                ...
                [[-0.3, 0.5, 0.4], [0.3, 0.5, 0.4], [-0.2, 0.8, 0.3], [0.2, 0.8, 0.3]].forEach(([dx, tz, len]) => {
