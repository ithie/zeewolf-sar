...
export const animMainMenuBg = () => {
    ...
    const c = document.getElementById('main-menu-bg-canvas') as HTMLCanvasElement;
    if (!c) return;
...
export const drawMenuHeli = () => {
    ...
    const c = document.getElementById('menu-heli-big') as HTMLCanvasElement;
    if (!c) return;
    const cx = c.getContext('2d');
    c.width = 800;
    c.height = 300;
...
export const animateHeliPreviews = () => {
    ...
    HELI_TYPES.forEach(ht => {
        ...
        const c = document.getElementById('icon-' + ht.id) as HTMLCanvasElement;
        if (c) {
            const cx = c.getContext('2d');
            c.width = 300;
            c.height = 200;
